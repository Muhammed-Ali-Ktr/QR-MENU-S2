...
router.get('/restaurant', (req, res) => {
  ...
  db.all('SELECT key, value FROM settings WHERE key IN (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', 
    ['restaurant_name', 'restaurant_description', 'restaurant_logo', 'restaurant_icon', 'restaurant_slogan', 'wifi_name', 'wifi_password', 'company_name', 'company_logo', 'company_slogan'],
    (err, rows) => {
      if (err) {
        return res.status(500).json({ error: err.message });
      }
