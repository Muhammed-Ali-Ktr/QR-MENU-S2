...
router.post('/settings/company', authenticateToken, upload.fields([
  ...
  try {
    const db = dbModule.getDb();
    const { company_name, slogan, menu_url, restaurant_name, restaurant_slogan, admin_brand_text } = req.body;
    ...
    const promises = [
      updateSetting('company_name', company_name || resolvedName || ''),
      updateSetting('company_slogan', slogan || resolvedSlogan || ''),
      updateSetting('restaurant_name', resolvedName || company_name || ''),
      updateSetting('restaurant_slogan', resolvedSlogan || ''),
      menu_url ? updateSetting('menu_url', menu_url) : Promise.resolve()
    ];

    if (admin_brand_text !== undefined) {
      promises.push(updateSetting('admin_brand_text', admin_brand_text || 'ZİFT STUDİO'));
    }

    if (req.files?.logo) {
      const logoPath = `/uploads/${req.files.logo[0].filename}`;
      promises.push(updateSetting('company_logo', logoPath));
      promises.push(updateSetting('restaurant_logo', logoPath));
    }
    if (req.files?.icon) {
      const iconPath = `/uploads/${req.files.icon[0].filename}`;
      promises.push(updateSetting('company_icon', iconPath));
      promises.push(updateSetting('restaurant_icon', iconPath));
    }
